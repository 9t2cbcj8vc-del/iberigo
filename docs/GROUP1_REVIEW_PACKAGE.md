...
**Sprint 87 update — owner publication choice recorded. Status: Owner publication selected — technical launch pending.** The owner has chosen to proceed toward publishing all five Group 1 pages as owner-reviewed practical information, since no external legal/professional review is available. This is a decision record, not a technical launch: no `noindex, nofollow` has been removed, no sitemap or robots change has been made, no redirect has been added, and no homepage/navigation change has been made. **All five pages remain `noindex, nofollow`; no page is published; technical launch is pending a separate future sprint per `docs/GROUP1_TECHNICAL_LAUNCH_CHECKLIST.md`.**

**Sprint 88 update — technical launch PR prepared. Status: Technical launch PR prepared — preview review pending.** The technical launch changes have been made and opened as a PR into `main` (not merged): `noindex, nofollow` removed only from the five selected pages, the five pages added to `sitemap.xml`, `robots.txt` left unchanged (it already permitted the routes), no redirects added, no legacy guide migrated. All 16 other draft pages were verified to still be `noindex, nofollow`. No external legal/professional review was available; this launch remains owner-reviewed practical information, not legal, tax, immigration, financial or rental advice. **The PR must be preview-checked in the Netlify deploy preview before merge — it is not merged and no page is live yet.**

**Sprint 89 update — launch preview QA passed. Status: Launch preview QA passed — merge pending.** PR #16 (commit `7ff11a4`) was checked on its Netlify deploy preview. All five selected pages returned `200` with no DRAFT badge, no `noindex`/`nofollow`, correct canonical URLs, title/description/Open Graph metadata, a visible practical-information disclaimer, working CTA links, and no layout break at 1280px or 390px. The preview's `sitemap.xml` and `search-index.json` each contained exactly the five selected pages. Nine representative non-selected draft pages were confirmed still `noindex, nofollow` and absent from the sitemap. `robots.txt` was unchanged, `/guides/banking/` and `/guides/eu-registration/` still return `200`, and no redirects or legacy migrations were made. No issues were found and no fixes were needed. **This does not mean legal approval; PR #16 remains unmerged and no page is live-published.**

**Sprint 90 update — Group 1 launched. Status: Group 1 launched as owner-reviewed practical information.** PR #16 was squash-merged into `main` on 2026-07-03 (commit `1dc6b9b`). Production verification on `https://iberigo.eu` confirmed all five selected pages return `200`, `index, follow`, no DRAFT badge, correct canonical, title/description, and a visible disclaimer; production `sitemap.xml` and `search-index.json` each contain exactly the five selected pages; eight representative non-selected drafts remain `noindex, nofollow` and out of the sitemap; `robots.txt` is unchanged; `/guides/banking/` and `/guides/eu-registration/` still return `200`; no redirects or legacy migration were added; no homepage/navigation change was made. **This is not legal, tax, immigration, financial or rental advice and does not constitute professional approval.** Future recommendation, not actioned: add a homepage or navigation link to `/start-here/`.

**Sprint 92 update — homepage discovery link prepared. Status: Homepage discovery link prepared — preview review pending.** A small, calm link to `/start-here/` (with brief supporting copy, no legal/professional-advice framing) has been added to the homepage hero on `group1/post-launch-homepage-link`, styled with the existing `.secondary-action` class — no `styles.css` changes. No additional page was published: the five already-launched pages remain `index, follow`, all non-selected drafts remain `noindex, nofollow`, and `sitemap.xml`/`search-index.json`/`robots.txt` are unchanged. No redirects or legacy migration were made. **This PR has not been merged; preview review is required first.**

**Sprint 93 update — homepage discovery link preview QA passed. Status: Homepage discovery link preview QA passed — merge pending.** PR #18 (commit `7ed2612`) was checked on its Netlify deploy preview at 1280px and 390px: the homepage returns `200`, the new `/start-here/` link is visible and naturally placed under the hero cards, copy is calm with no legal/professional-advice implication, and there is no crowding, overflow, or style regression. `/start-here/` returns `200` and remains `index, follow`. All five launched pages remain `index, follow`; eight representative non-selected drafts remain `noindex, nofollow`; the preview's `sitemap.xml`, `search-index.json`, `robots.txt`, and `styles.css` are byte-identical to `main`. No redirects or legacy migration were made; `/guides/banking/` and `/guides/eu-registration/` both return `200`. No issues found, no fixes needed. **PR #18 remains unmerged.**

**Sprint 94 (revision) update — Move to Spain card link prepared. Status: Homepage Move to Spain link preview QA pending.** PR #18's standalone "Start here" block (merged and live in production) is being replaced: the homepage's existing "Move to Spain" card "Explore" button now links directly to `/start-here/` using the existing `.primary-action` style, and the standalone block has been removed from `index.html`. "Vacation" and "Live" cards are unchanged. No additional page was published: the five launched pages remain `index, follow`, non-selected drafts remain `noindex, nofollow`, and `sitemap.xml`/`search-index.json`/`robots.txt`/`styles.css` are unchanged. No redirects or legacy migration were made. This is prepared on a new PR (superseding the merged Sprint 92/93 approach); **preview review is required before merge.**

**Sprint 96 update — homepage Move to Spain link launched. Status: Homepage Move to Spain link launched.** PR #19 was squash-merged into `main` on 2026-07-03 (commit `105ede9`), after first being updated with `main` to pick up the Sprint 95A sitewide visual coherence audit (which recommended merging PR #19 as-is). Production verification on `https://iberigo.eu` confirmed the homepage returns `200`, the standalone "Start here" block is gone, the "Move to Spain" card links to `/start-here/`, and "Vacation"/"Live" cards are unchanged; `/start-here/` returns `200` and remains `index, follow`; all five launched pages remain `index, follow`; sampled non-selected drafts remain `noindex, nofollow`; production `sitemap.xml`, `search-index.json`, `robots.txt`, and `styles.css` are byte-identical to `main`; no redirects or legacy migration; `/guides/banking/` and `/guides/eu-registration/` both return `200`. **This is a homepage cleanup only — not legal, professional, or indexing approval.**
