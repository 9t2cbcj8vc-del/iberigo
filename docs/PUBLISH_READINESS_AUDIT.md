...
#### Guide card CTA alignment fix prepared (Sprint 97A)

A live visual bug was found on `/start-here/`'s "Choose your path" section (7 route cards): CTA buttons had inconsistent widths (intrinsic to label length) and did not sit at a consistent bottom position across cards in the same row. Root cause: `.guide-info-card` (the shared card component used across every Guide System v1 page, including "Choose your path," "Related Guides," and "Previous/Next Step" journey cards) had no flex layout, so its CTA button simply sat wherever the card's text content ended. Fixed entirely inside the scoped `guideCss()` in `scripts/guide-components.js` — no `styles.css` change: `.guide-info-card` is now `display: flex; flex-direction: column;`, and its CTA (`.guide-info-card > .guide-button`) gets `margin-top: auto; width: 100%;` so it's pushed to a consistent bottom position with a consistent width across every card in a grid. Verified across `/start-here/` (7-card grid) and the densest draft page, `/moving-to-spain/non-eu-citizens/` (8-card grid): every button in a row shares identical width and bottom offset, min-height stays ≥44px, long labels (e.g. "View the Family Member of an EU Citizen Roadmap") wrap to two lines cleanly, no horizontal overflow at 1280px/390px, and TOC/reading-time/disclaimer blocks are unaffected. No additional pages published, no indexing changes, no redirects, no legacy migration, `sitemap.xml`/`search-index.json`/`robots.txt`/homepage/`styles.css` all unchanged. Status: **Guide card CTA alignment fix prepared — preview QA pending.**

#### Guide card CTA alignment fix revised (Sprint 97B)

CSS alone was not enough: cards in the same row still looked uneven because CTA labels varied from 24 to 49 characters, so some buttons wrapped to a two-line, 82px-tall shape next to single-line 62px buttons in the same row. Fix: normalized the CTA labels themselves. `/start-here/`'s "Choose your path" (7 cards) and `/moving-to-spain/non-eu-citizens/`'s "Choose your route" (8 cards) now use one consistent label, "View roadmap" (with "Continue" for the single card that links to Start Here rather than a roadmap) — the card title already tells the reader which roadmap it is, so the button no longer needs to repeat the full name. The same problem existed sitewide in every page's "Related Guides"/"Previous Step"/"Next Step" journey cards (driven by the shared `guideSummaries` object in `scripts/generate-guide-system.js`); normalized those to "View roadmap" (roadmap-titled entries) or "View guide" (guide-titled entries), and updated `guideLink()`/`GuideLinkCard()`'s fallback defaults to match. All changes are confined to `scripts/generate-guide-system.js` and `scripts/guide-components.js` — no `styles.css` edit. Verified across `/start-here/`, `/moving-to-spain/non-eu-citizens/`, `/moving-to-spain/family-member-eu-citizen/`, `/moving-to-spain/work-in-spain/`, and `/moving-to-spain/settling-into-spain/`: every button in every grid now shares one width and one height (44px) with no wrapped outliers, no horizontal overflow at 1280px/390px, TOC/reading-time/disclaimer unaffected. No additional pages published, no indexing changes, no redirects, no legacy migration, `sitemap.xml`/`search-index.json`/`robots.txt`/homepage/`styles.css` unchanged. Status: **Guide card CTA alignment fix revised — preview QA pending.**

#### Guide card CTA alignment preview QA passed (Sprint 97C)

PR #21 (commit `d56bfcc`) was checked on its Netlify deploy preview at 1280px and 390px. `/start-here/`'s "Choose your path" grid: all 7 cards show "View roadmap" with identical 197×44px buttons, centered text, consistent bottom alignment, no wrapping, no overflow. Secondary pages all coherent: `/moving-to-spain/non-eu-citizens/`'s 8-card "Choose your route" grid (7× "View roadmap", 1× "Continue" for the Start-Here card) and its 3-card related-guides grid ("View guide" ×3); `/moving-to-spain/family-member-eu-citizen/`, `/moving-to-spain/work-in-spain/`, and `/moving-to-spain/settling-into-spain/` all show uniform 44px-height buttons with no wrapped 82px outliers; `/living-in-spain/opening-a-bank-account/` has no CTA-card grids and is unaffected. TOC, reading-time, and disclaimer confirmed present; all touch targets ≥44px on the densest mobile page. Launch safety: all 5 launched pages remain `index, follow`; sampled drafts remain `noindex, nofollow`; preview's `sitemap.xml`, `search-index.json`, `robots.txt`, `styles.css` byte-identical to `main` (the only homepage diff was Netlify's own preview-injection script and its standard pretty-URL rewriting, not a source change); `/guides/banking/` and `/guides/eu-registration/` both return `200`. No issues found, no fixes needed. Status: **Guide card CTA alignment preview QA passed — merge pending.** PR #21 remains unmerged.

#### Guide and homepage CTA alignment (Sprint 97D)

Confirmed distinction requested for this sprint: production `/start-here/` still shows the old, uneven "View the X Roadmap" labels (0 occurrences of "View roadmap" found on production), while PR #21's preview correctly shows the normalized labels (7 occurrences of "View roadmap") — this is expected, since PR #21 is not merged.

Separately, a live homepage CTA mismatch was found and fixed: after PR #19, the homepage's "Move to Spain" card (now `<a class="primary-action" href="/start-here/">`) rendered visibly larger than the "Vacation"/"Live" cards' `<button>` CTAs. Root cause (three compounding issues in `styles.css`): (1) `.situation-card--illustrated button` is a dedicated, smaller-sized rule for illustrated homepage cards that only matches `<button>` elements, so the new `<a>` fell through to the larger generic `.primary-action` rule; (2) a separate, later rule (`.situation-card button, .popular-card button, .choice-actions button { min-width: 144px; ... }`) overrides button-only sizing at equal specificity, so `<button>` and `<a>` diverged further; (3) native `<button>` carries `appearance: auto` (browser-native control sizing) while `<a>` does not.

Fix, scoped entirely to the existing `.situation-card--illustrated` selectors (no redesign, no global button/anchor reset): extended the illustrated-card button rule to also match `.situation-card--illustrated .primary-action`, added `appearance: none` to both, and added a small override (`min-width: 136px; margin-top: clamp(18px, 2vw, 24px)`) placed after the competing generic rule so illustrated cards win the cascade — deliberately without touching `width`, so the existing mobile media query's `width: 100%` rule for `.primary-action`/`button` continues to apply unchanged.

Verified: all three homepage Explore buttons now render identically (136×44px at 1280px, full-width 44px-tall at 390px, same vertical position within their cards); Move to Spain still links to `/start-here/`; Vacation/Live route-preset (`data-route-preset`) behavior is unchanged; no standalone Start Here block returned; no horizontal overflow at either breakpoint. Re-checked all five Guide System pages in scope — `.guide-*` classes are unaffected by this `styles.css` change, all CTA grids remain uniform (44px height, single width per grid, no wrapped 82px outliers).
...
#### Guide source-card styling restored (Sprint 97F)

**Old source-card styling audit:** `app.js`'s homepage wizard-result flow already implements a mature, safe source-category system, used nowhere in the new Guide System. `styles.css` defines `.gov-link` (a bordered card with a left accent bar, driven by a `--gov-accent`/`--gov-soft` CSS-variable pair), a small `.gov-link-badge` icon container (26×26px rounded square) holding either `.gov-link-bars` — three horizontal red/gold bars, evoking the Spain flag without reproducing it — or `.gov-link-stars` — a blue circle with a dotted gold ring, evoking the EU flag abstractly — and `.gov-link-copy` for title/subtitle text. `app.js` assigns 5 existing variants (`general`, `fee`, `social`, `health`, `eu`) by matching each link's URL against `govDomains`/`euDomains` domain lists (e.g. `sede.policia.gob.es`, `seg-social.es`, `dgt.gob.es`, `europa.eu`). This was judged safe to reuse: accent colors and small badges only, no logos, no literal flags.

**New Guide System implementation:** rather than touching shared `styles.css` selectors (which are also used by the homepage wizard), added a parallel, scoped variant system entirely inside `scripts/guide-components.js`: `classifySource(item)` (new function, matches each official source's URL hostname against the same category domains, or falls back to name-matching for sources with no single URL such as "Local Town Halls") and `SOURCE_CATEGORY_META` (tag label + 2–4 letter initials per category). `OfficialSources()` now renders each card as `.guide-info-card.guide-source-card.guide-source-card--{category}` with a small badge + uppercase category tag above the title. New CSS lives entirely inside `guideCss()` (the existing per-page scoped `<style>` block) — no `styles.css` edit.

**9 categories implemented**, each with a distinct accent (left border + badge color) and short initials: `government` (red #aa151b, "ES") reused directly from the legacy general-government accent; `police` (navy #1d3a5f, "PN"); `eu` (blue #1954a6, "EU") matching the legacy EU accent; `tax` (amber #a25b00, "AT") reused from the legacy fee accent; `social-security` (blue #1d5fa4, "SS") reused from the legacy social accent; `traffic` (orange #b5651d, "DGT"); `healthcare` (teal #2b8f6f, "MS") reused from the legacy health accent; `municipal` (brown-gold #8a6d3b, "LOC") for varying/no-single-URL sources like town halls; `generic` (calm default IberiGo rust #a64a36, "OS") as the fallback for anything unclassified.

**Examples verified after regeneration:** `/moving-to-spain/eu-citizens/` — Spanish Government (government/red), Ministry responsible for immigration (government/red), Police appointment portal (police/navy), Local Town Halls (municipal/gold). `/moving-to-spain/family-member-eu-citizen/` — adds EU official information for families (eu/blue). `/living-in-spain/taxes/` — Agencia Tributaria (tax/amber). `/living-in-spain/driving/` — DGT (traffic/orange). `/moving-to-spain/healthcare/` — Ministry of Health (healthcare/teal), Ministry responsible for immigration (government/red), Regional health services (municipal/gold), Social Security (social-security/blue).

**Visual QA** at 1280px/390px on the pages above plus `/moving-to-spain/documents-checklist/` and `/moving-to-spain/eu-registration/`: cards read as clearly source-categorized and trustworthy without impersonating any official site, consistent card padding/icon area/title-subtitle spacing preserved, clickable area obvious (underlined links, `↗` external-link marker unchanged), no horizontal overflow, mobile layout clean and readable. Confirmed no regression to Guide card CTA normalization/alignment, homepage Move/Vacation/Live CTA consistency, or the Draft-label removal from Sprint 97E — all still hold.
...
#### PR #21 final preview QA passed (Sprint 98)

PR #21 (commit `0bfbc9c`) was checked on its Netlify deploy preview (`https://deploy-preview-21--iberigo.netlify.app`, deploy ready) at 1280px and 390px, covering every fix bundled in the PR.

- **Homepage:** Move/Vacation/Live Explore buttons all identical — 136×44px at 1280px, full-width 286×44px at 390px. Move to Spain links to `/start-here/`; Vacation/Live `data-route-preset` behavior unchanged; no standalone Start Here block returned; header, search icon, language switcher, footer, and Donate link all render normally; no overflow at either breakpoint.
- **Guide CTA:** `/start-here/`, `/moving-to-spain/non-eu-citizens/`, `/moving-to-spain/family-member-eu-citizen/`, `/moving-to-spain/work-in-spain/`, `/moving-to-spain/settling-into-spain/` all show equal-width, equal-height (44px), bottom-aligned CTAs using "View roadmap"/"View guide"/"Continue" as appropriate, no wrapped outliers, no overflow; TOC, reading-time, and disclaimer confirmed unaffected on all five.
- **Draft-label QA:** zero visible `.guide-status-badge` elements on `/start-here/`, `/moving-to-spain/eu-citizens/`, `/moving-to-spain/work-in-spain/`, `/living-in-spain/taxes/`, and `/search/`; `robots` meta confirmed still correctly `index, follow` or `noindex, nofollow` on each, unaffected by the badge removal.
- **Official source-link QA:** `/moving-to-spain/documents-checklist/` (police/government), `/living-in-spain/opening-a-bank-account/` (no source cards, unaffected), `/moving-to-spain/eu-registration/` (government/police), `/moving-to-spain/family-member-eu-citizen/` (government/police/eu), `/living-in-spain/taxes/` (tax), `/living-in-spain/driving/` (traffic), and `/moving-to-spain/healthcare/` (healthcare/government/municipal/social-security) all show coherent, subtly-accented, correctly-categorized cards; no wrapping at 390px; no overflow.
- **Regression QA:** homepage, `/the-spain-files/`, `/support/` (Donate), `/guides/banking/`, `/guides/eu-registration/` all render normally with no button-size or layout regression; both legacy routes return `200`.
