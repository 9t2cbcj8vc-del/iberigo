...
The shared script and index use matching `20260712-sitewide-4` request versions. `_headers` gives the script a one-day revalidating cache and the index a five-minute revalidating cache, limiting mismatched deployments without disabling asset caching broadly.
