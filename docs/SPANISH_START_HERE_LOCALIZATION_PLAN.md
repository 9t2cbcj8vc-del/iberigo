...
**Status:** Spanish Guide System launched. Route roadmap family (7 English/Spanish pairs) content-reviewed, official-source-verified, owner-approved, and now launched (PR #55, commit `b01d574`, 2026-07-11). See `docs/SPANISH_START_HERE_REVIEW_AND_LAUNCH_PLAN.md`, `docs/ROUTE_ROADMAP_LEGAL_REVIEW_BRIEF.md`, `docs/ROUTE_ROADMAP_SOURCE_VERIFICATION_MATRIX.md`, and `docs/ROUTE_ROADMAP_PUBLICATION_DECISION.md`.
**Sprint:** 124
**Date:** 2026-07-04
...
**Sprint 134 update (2026-07-05):** PR #39 was squash-merged into `main` (commit `ab79f31`) and verified live on production. `/es/moving-to-spain/documents-checklist/` is now a live draft (`noindex, nofollow`, absent from `sitemap.xml`/`search-index.json`). English pages, all 5 launched pages, and `/es/start-here/` all confirmed unaffected. No other Spanish Group 1 page was created; no hreflang or public language-switcher launch behavior was added; `app.js`/`styles.css` unchanged.

**Sprint 136 update (2026-07-06):** created `/es/moving-to-spain/finding-accommodation/`, the second linked Spanish Group 1 draft. It is `noindex, nofollow`, absent from `sitemap.xml`/`search-index.json`, has no hreflang, and has no public language-switcher launch behavior. Only `scripts/generate-guide-system.js` was changed (new route, Spanish content and metadata); `app.js` and `styles.css` were not touched. No other Spanish Group 1 page was created. Preview QA is required before merge.

**Sprint 137 update (2026-07-06):** PR #41 preview QA passed on `https://deploy-preview-41--iberigo.netlify.app`. No issues found: route returns `200` with correct Spanish metadata and `noindex, nofollow`; content review confirmed calm, consistent terminology and no overstated legal/timeline/province certainty; visual QA at 1280px/390px found no overflow; both existing Spanish drafts, English pages, and all 5 launched pages unaffected; legacy audit stable at 4/70. PR #41 remains unmerged.

**Sprint 138 update (2026-07-06):** PR #41 was squash-merged into `main` (commit `22f5118`) and verified live on production. `/es/moving-to-spain/finding-accommodation/` is now a live draft (`noindex, nofollow`, absent from `sitemap.xml`/`search-index.json`). English pages, all 5 launched pages, `/es/start-here/`, and `/es/moving-to-spain/documents-checklist/` all confirmed unaffected. No other Spanish Group 1 page was created; no hreflang or public language-switcher launch behavior was added; `app.js`/`styles.css` unchanged.
...
**Route roadmap official-source verification prepared — owner decision pending (2026-07-11):** a follow-up brief (`docs/ROUTE_ROADMAP_LEGAL_REVIEW_BRIEF.md`) formally requested a qualified professional legal/immigration review. Since that review remained unavailable, an official-source verification pass was performed instead, as a documented owner-controlled substitute — see `docs/ROUTE_ROADMAP_SOURCE_VERIFICATION_MATRIX.md`. Checked all 14 pages' major claims against official sources only (BOE, `inclusion.gob.es`, `exteriores.gob.es`, `seg-social.es`, `sede.policia.gob.es`): 7 of 7 topics verified with no unsupported claims; 1 of 7 (Students) had its work-rights wording updated in both languages to reflect a real, sourced regulatory change (RD 1155/2024, in force 2026-05-20) without stating an exact hour figure. **This is not a substitute for professional legal advice and is not legal advice.** Launch still requires either professional review or an explicit owner publication decision (per `docs/GROUP1_PUBLICATION_DECISION.md`'s precedent) — neither has happened. All 14 pages remain `noindex, nofollow`.

**Route roadmap owner publication decision prepared — launch PR next (2026-07-11):** the owner reviewed the verification matrix above and made an explicit decision — `docs/ROUTE_ROADMAP_PUBLICATION_DECISION.md` — to prepare the route roadmap family for publication without professional review, on the basis of the official-source verification, accepting the residual risk that entails. The decision covers only these 14 pages, states plainly it is not legal advice, and sets out the technical conditions (index/follow, sitemap, search-index, hreflang, language switcher, `/es/start-here/` link updates, selective "(en inglés)" removal, no redirects, no legacy migration) a future, separate launch PR must satisfy. **No launch happened as part of this decision** — all 14 pages remain `noindex, nofollow` until that separate launch PR is opened and merged.

**Route roadmap launch prepared — preview QA pending (2026-07-11):** that separate launch PR (`guide-system/route-roadmap-launch`) has now been prepared. All 14 pages changed to `index, follow`; `sitemap.xml` grew from 87 to 101 total `<loc>` entries; `search-index.json` grew from 10 to 24 entries; reciprocal hreflang and a working language switcher added for the 7 pairs via the existing `launchedPair()`/`altHref` mechanism (no `app.js`/`styles.css` change needed). `/es/start-here/`'s 7 roadmap cards and the EU roadmap card in "La mayoría de las personas empieza aquí" now link to Spanish routes with "(en inglés)" removed from exactly those 8 cards; deeper English-only links keep their label everywhere, confirmed via automated audit. **Not yet merged** — preview QA is pending before this PR can be merged.

**Route roadmap pages launched (2026-07-11):** PR #55 squash-merged into `main` (commit `b01d574`); branch protection required approving reviews temporarily set 1 → 0 → 1. Production verification confirmed all 14 pages live, `index, follow`, correct reciprocal hreflang, working language switcher, `sitemap.xml` at 101 total `<loc>` entries, `search-index.json` at 24 entries, `/es/start-here/` cards updated correctly, deeper English-only links still labeled, `robots.txt`/`app.js`/`styles.css` unchanged, legacy audit remains exactly 4/70. This completes the route roadmap localization and launch effort, following the official-source verification and owner publication decision documented above.
