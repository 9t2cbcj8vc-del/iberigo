...
// guides/ and guides/es/ also hold Guide System v1 pages generated by
// scripts/generate-guide-system.js (e.g. the vacation-in-spain and
// living-in-spain hub pages added in PR #117) -- these live under the same
// directory for URL-collision reasons but are not part of the hand-maintained
// legacy-guide universe this audit tracks. Legacy pages always carry a
// data-guide-id attribute on <html>; generated Guide System v1 pages never
// do. A topic with no index.html at all is left in scope (it's exactly the
// "missing-index-html" case this audit exists to catch), so only topics
// whose file exists and explicitly lacks data-guide-id are excluded.
function isGuideSystemV1Page(topic, lang) {
  const relDir = lang === "en" ? `guides/${topic}` : `guides/es/${topic}`;
  const filePath = path.join(REPO_ROOT, relDir, "index.html");
  if (!fs.existsSync(filePath)) return false;
  const html = fs.readFileSync(filePath, "utf8");
  return !extractDataGuideId(html);
}

function listLegacyTopicDirs(dir, lang) {
  return listTopicDirs(dir).filter((topic) => !isGuideSystemV1Page(topic, lang));
}
...
function run() {
  ...
  const englishTopics = listLegacyTopicDirs(GUIDES_DIR, "en");
  const spanishDir = path.join(GUIDES_DIR, "es");
  const spanishTopics = fs.existsSync(spanishDir) ? listLegacyTopicDirs(spanishDir, "es") : [];
